  ...
  return function (selector, items) {
    ...
    input.addEventListener('keyup', function (ev) {
        ...
        for (var i = 0, len = liChildren.length; i < len; i+=1) {
            ...
            var childHeadText = currLi.firstElementChild.innerHTML ;
    ...
    for (var i = 0, len = items.length; i < len; i += 1) {
        ...
        currentLI.className += 'image-container';
        var currentHeaderLI = headerLI.cloneNode(true);
        currentHeaderLI.innerHTML  = currentTitle;
    ...
    ul.addEventListener('click', function (ev) {
        ...
        if (target.tagName === 'IMG') {
            header.innerHTML  = target.previousElementSibling.innerHTML ;
